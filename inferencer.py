...
from dataset import spacenet_data_partition, sentinel2_data_partition, s2rosa_data_partition, s2roads_data_partition
...
if __name__ == "__main__":
    ...
    elif config.DATASET == 's2roads':
        _, _, test_img_indices = s2roads_data_partition()
        base_dir = './s2roads_data'
        rgb_pattern = os.path.join(base_dir, 'images/{}.png')
        gt_graph_pattern = os.path.join(base_dir, 'graphs_p/{}.p')
        gt_mask_pattern = os.path.join(base_dir, 'road_masks/{}.png')
        print(f'Running inference on {len(test_img_indices)} test images')
    ...
    for img_id in test_img_indices:
        ...
        # only for sentinel2 / s2rosa / s2roads which have gt masks, for cityscale/spacenet we can only compare with gt graph
        if config.DATASET in ('sentinel2', 's2rosa', 's2roads'):
        ...
        # gt_mask is only defined in sentinel2 / s2rosa / s2roads dataset if statement
        if config.DATASET in ('sentinel2', 's2rosa', 's2roads'):
